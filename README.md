...
canonical route. What a text alias, a decision alias and a media alias each
promise, and what the catalogue says about a model, is documented in
...
- `brama subscription sign-in-manual`: the provider's own OAuth flow, run here
  — same client id, scopes, loopback callback and token endpoint a coding
  harness uses — minting a pair that belongs to this gateway. Taking a grant a
  harness already holds is deliberately absent: one pair has one holder, and a
  second holder costs the first its session.
- `brama subscription disown`: give a pool member back, for members left from
  when borrowing existed.
...
- Every credential read, write, listing and capability issue runs one vault
  process. Which program that is: `ENTITLEMENTS_ROUTER_BIN`, then
  `SKARBIEC_BIN`, then `entitlements-router` or `skarbiec` on `PATH`, then
  `$HOME/.stado/bin/skarbiec`. A host carrying none of them is refused with the
  program that was run and both variable names, never with a bare
