...
mod tests {
    ...
    #[test]
    fn parses_visible_codex_models_from_app_server() {
        let line = r#"{"id":2,"result":{"data":[{"id":"gpt-5.6-sol","hidden":false,"inputModalities":["text","image"],"supportedReasoningEfforts":[{"reasoningEffort":"high"}]},{"id":"retired","hidden":true,"inputModalities":["text"],"supportedReasoningEfforts":[]}]}}"#;
        let models = parse_codex_model_list_line(line)
            .expect("response must parse")
            .expect("model/list response must be recognized");
    ...
    #[test]
    fn only_the_subscription_alias_uses_codex_default_model() {
        assert_eq!(codex_model_override(CODEX_SUBSCRIPTION_MODEL), None);
        assert_eq!(codex_model_override("gpt-5.6-terra"), Some("gpt-5.6-terra"));
    }

    #[tokio::test]
    #[ignore = "requires a live local Codex ChatGPT credential"]
    async fn live_codex_model_catalog_uses_account_entitlements() {
        let home = std::env::var("HOME").expect("HOME must be set");
        let token_json = fs::read_to_string(Path::new(&home).join(".codex/auth.json"))
            .await
            .expect("local Codex auth.json must exist");

        let models = list_codex_models(&token_json)
            .await
            .expect("Codex model/list must succeed");

        assert!(models.iter().any(|model| model.id == "gpt-5.6-sol"));
        assert!(models.iter().all(|model| valid_codex_model_id(&model.id)));
    }
}
