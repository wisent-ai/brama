...
pub(crate) const DEFAULT_CATALOG_URL: &str = "https://models.dev/api.json";

pub(crate) const DEFAULT_CACHE_PATH: &str = "/tmp/brama-models-dev-cache.json";

pub(crate) const DEFAULT_TTL_SECONDS: u64 = 900;
...
pub(crate) static MEMORY_CACHE: LazyLock<RwLock<Option<CachedSnapshot>>> = LazyLock::new(|| RwLock::new(None));

pub(crate) static LOAD_LOCK: LazyLock<Mutex<()>> = LazyLock::new(|| Mutex::new(()));
...
pub(crate) fn catalog_url() -> Result<reqwest::Url, String> {
...
pub(crate) async fn read_live_catalog() -> Result<String, String> {
...
pub(crate) async fn read_cache() -> Result<String, String> {
    tokio::fs::read_to_string(cache_path())
        .await
        .map_err(|error| error.to_string())
}

pub(crate) async fn write_cache(raw: &str) -> Result<(), String> {
    let path = cache_path();
...
pub(crate) fn parse_catalog(raw: &str) -> Result<CatalogSnapshot, String> {
