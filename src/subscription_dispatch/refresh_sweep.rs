...
/// Walk every active subscription once, silently refreshing grants that are due.
///
/// A failed or absent grant is only reported and left for an explicit,
/// account-selected sign-in. A timer must never choose a Weles account or open
/// an authentication flow as a side effect of credential maintenance.
async fn sweep(skew: Duration) {
    ...
    info!(
        ...
        refused,
        awaiting_signin,
        "finished one credential refresh sweep; accounts awaiting sign-in require an explicit \
         selected-account action"
    );
}
    ...
    match broker::refresh_subscription_credential_ahead(&subscription_id, &provider, skew).await {
        ...
        RefreshAhead::Unavailable(refused) => {
            warn!(
                ...
                "no credential could be read for this subscription; an operator must explicitly \
                 select its declared Weles account to restore the grant"
            );
            Swept::AwaitingSignIn
        }
    }
