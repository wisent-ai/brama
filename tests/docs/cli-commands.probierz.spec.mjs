// Measure the real CLI, the registered HTTP router and their canonical pages.
...
export function runHelp(binary, argv) {
  return execFileSync(binary, ["help", ...argv], { encoding: "utf8", env: { ...process.env, ...HELP_ENV } }).replaceAll("\r\n", "\n");
}
export function buildIdentity(binary) {
  const identity = JSON.parse(execFileSync(binary, ["version"], { encoding: "utf8", env: { ...process.env, NO_COLOR: "1" } }));
...
function rows(help, name, pattern) {
  return section(help, name).map((line) => line.match(pattern)).filter((match) => match !== null).map((match) => match[1].trim());
}
export function parseHelp(help, tokens = []) {
  ...
  return {
    ...
    options: rows(help, "Options", /^\s{2,}(-\S(?:.*?\S)?)(?:\s{2,}|$)/).filter((syntax) => !/(^|,\s*)-(?:h|-help|V|-version)\b/.test(syntax)),
  };
}
...
export function optionMirrors(leaves) {
  ...
  return leaves.flatMap((host) => names.filter((name) => name !== host.tokens.at(-1)).map((name) => ({
    host: host.tokens.join(" "), mirrored: name, options: host.options.filter((syntax) => syntax.startsWith(`--${name}-`)),
...
export function parseRouterInvocations(source) {
  ...
  for (let at = source.indexOf(marker); at >= 0; at = source.indexOf(marker, at + 1)) {
    ...
    const methods = [...args.matchAll(/\b([a-z]+)\s*\(/g)].map((match) => match[1]).filter((name) => METHODS.includes(name));
    if (methods.length === 0) throw new Error(`.route("${path[1]}") names no HTTP method`);
    for (const method of methods) invocations.push({ method: method.toUpperCase(), path: path[1], group: httpGroup(path[1]) });
    at = end;
  }
  return invocations;
}
...
export function readDocsRegistry(ingress = INGRESS) {
  ...
  const docs = config.rewrites.filter((r) => typeof r.source === "string" && typeof r.destination === "string" && r.destination.startsWith("/docs/"));
...
export const FAMILIES = [
  { name: "credential repair, once per incident", capability: "subscription credential state",
    declaration: "the usage ledger's credential state and the journal's subscription_refresh records",
    matches: ({ path = "", tokens = [] }) => /sign-in|-pool\/refresh$|\/probe$/.test(path) || (tokens[0] === "subscription" && ["refresh", "sign-in"].includes(tokens[1])), },
  { name: "plan-usage reading, once per audience", capability: "subscription plan usage",
    declaration: "the usage ledger and the provider's own usage report", matches: ({ path = "" }) => /subscription-usage|-pool\/usage$/.test(path), },
  { name: "subscription inventory, once per audience", capability: "the subscription pool",
    declaration: "the vault entitlement declaration: brama:subscription with its agent, provider and id tags",
    matches: ({ path = "", tokens = [] }) => /^\/v1\/(subscriptions|account\/subscriptions|admin\/subscription)/.test(path) || (tokens[0] === "subscriptions" && tokens[1] === "list"), },
  { name: "route adoption, once per entry point", capability: "the alias registry",
    declaration: "the route registry BRAMA_INFERENCE_ROUTES_FILE names and the launcher table BRAMA_MODEL_ALIASES",
    matches: ({ path = "", tokens = [] }) => /configuration-adoption|^\/v1\/admin\/routes$/.test(path) || tokens[0] === "adopt", },
  { name: "provider credential administration", capability: "capability redemption at final use",
    declaration: "the provider capability declaration this installation was given", matches: ({ path = "" }) => path === "/v1/admin/credentials", },
];
...
export function measure({ binary, repository = REPOSITORY, ingress = INGRESS, readHelp = (t) => runHelp(binary, t) } = {}) {
  ...
  const cli = leaves.map((leaf) => ({ name: `brama ${leaf.tokens.join(" ")}`, invocation: leaf.invocation,
    options: leaf.options.length, route: cliRoute(leaf.tokens), documented: pageFor(cliRoute(leaf.tokens), registry, ingress) !== null, family: classify({ tokens: leaf.tokens }), }));
  ...
  const apiDirectory = join(ingress, "docs/http-api");
  const apiText = [readFileSync(apiPage, "utf8"), ...readdirSync(apiDirectory).filter((name) => name.endsWith(".html"))
    .map((name) => readFileSync(join(apiDirectory, name), "utf8"))].join("\n");
  const router = readFileSync(join(repository, "src/core/server/lifecycle/router.rs"), "utf8");
  const http = parseRouterInvocations(router).map((one) => ({ ...one, name: `${one.method} ${one.path}`,
    documented: apiText.includes(one.path), family: classify(one), }));
  ...
  return {
    identity: buildIdentity(binary), cli, http, mirrors: optionMirrors(leaves),
    families: FAMILIES.map((f) => ({ ...f, members: named(all.filter((o) => o.family === f.name)) })),
    primitives: named(all.filter((one) => one.family === null)), undocumented: named(all.filter((one) => !one.documented)),
    unrouted: [...registry.keys()].filter((r) => !r.includes(":") && pageFor(r, registry, ingress) === null),
    groups: [...groups.map((g) => ({ name: g.tokens.length > 0 ? `brama ${g.tokens.join(" ")}` : "brama (root)",
      route: cliRoute(g.tokens), count: leaves.filter((leaf) => leaf.tokens.slice(0, -1).join("/") === g.tokens.join("/")).length,
    })), ...[...httpGroups.entries()].map(([name, count]) => ({ name, count }))].sort((left, right) => right.count - left.count || left.name.localeCompare(right.name)),
    counts: { leaves: all.length, cliLeaves: cli.length, cliGroups: groups.length, cliOptions: cli.reduce((total, leaf) => total + leaf.options, 0),
      httpInvocations: http.length, httpPaths: new Set(http.map((one) => one.path)).size, groups: groups.length + httpGroups.size, registeredDocsRoutes: registry.size, },
  };
}
export function formatReport(m) {
  return [
    ...
    "groups:", ...m.groups.map((g) => `  ${String(g.count).padStart(3)}  ${g.name}`),
    "families:", ...m.families.flatMap((f) => [`  ${String(f.members.length).padStart(3)}  ${f.name} -> ${f.capability} (reads ${f.declaration})`, ...f.members.map((one) => `        ${one}`)]),
...
test("documentation covers exactly the commands the binary has", () => {
  const m = surface();
  assert.deepEqual(m.cli.filter((leaf) => !leaf.documented).map((leaf) => leaf.route), [], "a command with no resolving page is undiscoverable to whoever has to run it");
  const commandRoutes = [...m.cli, ...m.groups].map((one) => one.route);
  const routes = new Set(commandRoutes.filter((route) => route !== undefined));
  assert.deepEqual([...readDocsRegistry().keys()].filter((s) => s.startsWith("/docs/cli/") && !routes.has(s)), [], "a page for a removed command keeps promising it");
});
test("the production site serves every measured command page", async (t) => {
  for (const route of [...surface().cli.map((leaf) => leaf.route), "/docs/command-surface"]) {
    await t.test(route, async () => {
      ...
      assert.match(await response.text(), new RegExp(`<link\\s+rel="canonical"\\s+href="${escaped}"`), `${route} must declare its canonical URL`);
    });
  }
});
