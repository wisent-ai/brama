...
fn run_stage(label: &str) -> (String, serde_json::Value) {
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&router, fs::Permissions::from_mode(0o755)).expect("router is executable");
    }
    ...
    assert!(output.status.success(), "the stage failed: {}",
...
fn a_subscription_the_policy_names_is_served_and_one_it_does_not_is_reported() {
    ...
    let served: Vec<&str> = items.iter().filter_map(|item| item["id"].as_str()).collect();
    assert_eq!(served, ["brama-sub-wisent-app-claude-primary"],
               "only the subscription the policy names is served: {served:?}");
    assert_eq!(items[0]["login_item"].as_str(), Some("weles-google-sso-login"),
               "the account that can renew it travels with it");

    assert!(log.contains("provider:codex:brama-sub-wisent-app-codex-primary"),
            "the dropped subscription is not named: {log}");
    assert!(log.contains("no capability route maps this resource to a vault item and field"),
            "the log does not say what the gateway will answer later: {log}");
    assert!(log.contains("install this release again on this host"),
            "the log does not name the remedy: {log}");
    assert!(log.contains("1 subscription(s) the runtime policy does not name"),
            "the boot log has no count: {log}");
    assert!(log.contains("1 subscription(s) served by this gateway"),
...
fn an_item_without_an_id_tag_says_what_is_missing() {
    ...
    assert!(log.contains("brama:id:"), "the missing tag is not named: {log}");
}
